...
// 判断系统和判断是否有返回值
window.executeNative = function({ methodName, params, hasReturn }) {
	if (isAndroid) {
		if (hasReturn) {
...
window.executeAndroidMethodWithReturn = function(methodName, params) {
	return new Promise((resovle, reject) => {
		if (window.android[methodName]) {
			if (params) {
				resovle(window.android[methodName](params))
			} else {
				resovle(window.android[methodName]())
			}
		} else {
			reject('安卓没有注入此方法')
		}
